    ...
    try:
        ...
        for record in root.findall(".//oai:record", ns):
            try:
                ...
                if not all(
                    [
                        e is not None and e.text is not None
                        for e in [identifier_elem, abstract_url_elem, title_elem, abstract_elem, date_elem]
                    ]
                ):
                ...
                for creator in record.findall(".//dc:creator", ns):
                    if creator.text:
                        ...
                        authors.append(
                            {"last_name": name_parts[0], "first_name": name_parts[1] if len(name_parts) > 1 else ""}
                        )
...
def create_research_summary(records: list, date: str) -> dict:
    ...
    for category in CATEGORIES:
        ...
        if category_papers:
            ...
            summary_files[category] = {"s3_key": s3_key, "papers": category_papers}
