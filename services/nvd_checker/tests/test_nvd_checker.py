    ...
    def get_parameters(self, Names, WithDecryption=True):
        return {
            "Parameters": [
                ...
                {
                    "Name": f"{os.environ['CONFIG_PATH']}/nvd/monitored_systems",
                    "Value": json.dumps(self.config["monitored_systems"]),
                },
...
fake_requests.exceptions = type("Exceptions", (), {"RequestException": Exception, "JSONDecodeError": Exception})
...
def test_init_with_valid_config():
    ...
    assert checker.headers == {"apiKey": "test_api_key", "User-Agent": "AtomikLabs-VulnChecker/1.0"}
            ...
            "vulnerabilities": [
                {
                    ...
                    "metrics": {"baseScore": 7.5, "baseSeverity": "HIGH"},
                    "product": "product1",
                }
            ],
        }
