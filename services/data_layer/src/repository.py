...
from .models import Article, Author, Category, Email, Newsletter, Organization, ProcessingEvent
...
class ArticleRepository:
    ...
    @staticmethod
    def get_article_by_source_id(session: Session, source: str, source_id: str) -> Optional[Article]:
        """
        ...
        Returns:
            Article or None: Found article or None if not found
        """
        return session.query(Article).filter(Article.source == source, Article.source_id == source_id).first()

    @staticmethod
    def add_author_to_article(session: Session, article_id: int, author_id: int, order: int) -> None:
        """
        ...
        stmt = session.query(article.authors.through).filter_by(article_id=article_id, author_id=author_id)

        if not session.query(stmt.exists()).scalar():
            # Add association with order
            session.execute(
                article.authors.through.insert().values(article_id=article_id, author_id=author_id, author_order=order)
            )
            session.commit()

    @staticmethod
    def add_category_to_article(session: Session, article_id: int, category_id: int, is_primary: bool = False) -> None:
        """
        ...
        # Check if association already exists
        stmt = session.query(article.categories.through).filter_by(article_id=article_id, category_id=category_id)
        ...
        if not session.query(stmt.exists()).scalar():
            ...
            session.execute(
                article.categories.through.update()
                .where(
...
class AuthorRepository:
    ...
    @staticmethod
    def create_author(session: Session, name: str, email: Optional[str] = None) -> Author:
        """
    ...
    @staticmethod
    def add_organization_to_author(session: Session, author_id: int, organization_id: int) -> None:
        """
        ...
        """
        author = AuthorRepository.get_author(session, author_id)
        organization = OrganizationRepository.get_organization(session, organization_id)
...
class OrganizationRepository:
    ...
    @staticmethod
    def create_organization(session: Session, name: str, country: Optional[str] = None) -> Organization:
        """
    ...
    @staticmethod
    def get_organization(session: Session, organization_id: int) -> Optional[Organization]:
        """
...
class CategoryRepository:
    ...
    @staticmethod
    def create_category(session: Session, name: str, code: str, parent_id: Optional[int] = None) -> Category:
        """
...
class ProcessingEventRepository:
    ...
    def create_event(
        ...
        event = ProcessingEvent(
            article_id=article_id, event_type=event_type, details=details, source_job_id=source_job_id
        )
...
class NewsletterRepository:
    ...
    @staticmethod
    def create_newsletter(session: Session, title: str, issue_date: date, s3_path: str) -> Newsletter:
        """
        ...
        """
        newsletter = Newsletter(title=title, issue_date=issue_date, s3_path=s3_path)
    ...
    def add_article_to_newsletter(
        ...
        # Check if association already exists
        stmt = session.query(newsletter.articles.through).filter_by(newsletter_id=newsletter_id, article_id=article_id)

        if not session.query(stmt.exists()).scalar():
            # Add association with section and priority
            session.execute(
                newsletter.articles.through.insert().values(
                    newsletter_id=newsletter_id, article_id=article_id, section=section, priority=priority
                )
            )
            session.commit()
    ...
    def get_newsletter(session: Session, newsletter_id: int) -> Optional[Newsletter]:
        ...
        """
        return session.query(Newsletter).filter(Newsletter.id == newsletter_id).first()

    @staticmethod
    def get_newsletter_by_date(session: Session, issue_date: date) -> Optional[Newsletter]:
        """
        ...
        Returns:
            Newsletter or None: Found newsletter or None if not found
        """
        return session.query(Newsletter).filter(Newsletter.issue_date == issue_date).first()
...
class EmailRepository:
    ...
    @staticmethod
    def create_email(
        session: Session, subject: str, newsletter_id: int, recipient_count: int, s3_path: Optional[str] = None
    ) -> Email:
        """
        ...
        """
        email = Email(subject=subject, newsletter_id=newsletter_id, recipient_count=recipient_count, s3_path=s3_path)
